...
var getDb = function (host, done) {
  ...
  var mongoOptions = {};
  host = host || localhost;
  console.log("mongo host is", host)
...
var initReplSet = function (db, hostIpAndPort, done) {
  ...
  db.admin().command({ replSetInitiate: {} }, {}, function (err) {
    ...
    // 上一条命令执行完之后需要休眠5秒，再在primary节点执行相关操作
    setTimeout(() => {
      // 如果有配置用户，需要新建用户
      if (config.username) {
        db.admin().authenticate(config.username, config.password, function (err) {
          // 使用配置的用户登录失败
          if (err) {
            // 创建新用户
            db.admin().command({ createUser: config.username, pwd: config.password, roles: ["root"] }, function (err, result) {
              if (err) {
                return done(err);
              } else {
                ...
                db.authenticate(config.username, config.password, function (err, result) {
                  if (err) {
                    ...
                    replSetGetConfig(db, function (err, rsConfig) {
                      ...
                      async.retry({ times: 20, interval: 500 }, function (callback) { replSetReconfig(db, rsConfig, false, callback); }, function (err, results) {
                        if (err) {
                          return done(err);
                        }
                        return done();
